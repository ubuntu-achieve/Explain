"""
Code from: https://github.com/ankurtaly/Integrated-Gradients
...
def ConvertToColor(img, channel):
    img_color = np.zeros((img.shape[0], img.shape[1], 3))
    img_color[:, :, channel] = img
    return img_color


# Function to plot different digit figures in the EIG paper.
def ConvertToColor_img(img, mult):
    img_color = np.ones((img.shape[0], img.shape[1], 3)) * mult
    for i in range(img.shape[0]):
        tmp = np.where(img[i, :] > 0)[0]
        img_color[i, tmp, 0] = 0
        img_color[i, tmp, 1] = 72
        img_color[i, tmp, 2] = 186
    return img_color
