...
def save_heatmap(output_path, mask, img, blurred, blur_mask=0):
    ...
    cv2.imwrite(os.path.join(output_path, "heatmap.png"), np.uint8(255 * heatmap))
    cv2.imwrite(os.path.join(output_path, "IGOS.png"), np.uint8(255 * IGOS))
    cv2.imwrite(os.path.join(output_path, "blurred.png"), np.uint8(255 * blurred))
