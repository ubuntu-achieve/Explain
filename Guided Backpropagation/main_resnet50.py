...
from keras.applications.resnet50 import ResNet50, preprocess_input, decode_predictions, resnet50
...
def compute_saliency(model, guided_model, img_path, layer_name='block5_conv3', cls=-1, visualize=True, save=True):
    ...
    return deprocess_image(guided_gradcam[0])
...
root_path = 'Images'
# 保存图片路径
output_path = 'Results/Guided Backpropagation'
...
if __name__ =="__main__":
    ...
    for img_name in os .listdir(root_path):
        guided_backpropagation = compute_saliency(
            model,
            guided_model,
            os.path.join(root_path, img_name),
            layer_name='block5_conv3',
            cls=-1,
            visualize=is_show,
            save=True
        )
        cv2.imwrite(os.path.join(output_path, 'guided_backpropagation_resnet50_'+img_name), guided_backpropagation)